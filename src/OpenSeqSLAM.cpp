...
//
// This implementation is based on https://github.com/subokita/OpenSeqSLAM
...
Mat OpenSeqSLAM::calcDifferenceMatrix(vector<Eigen::VectorXd> &set_1, vector<Eigen::VectorXd> &set_2)
{
    int n = static_cast<int>(set_1.size());
    int m = static_cast<int>(set_2.size());
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            diff_ptr[j] = (set_2[j] - set_1[i]).cwiseAbs().sum() / (double)n;
        }
        // diff_ptr[j] = sum( abs(set_2[j] - set_1[i]) )[0] / n;
    }
...
/**
 * Apply OpenSeqSLAM to sets of images represented as an intensity vector
 *
 * First set is the original sequence of images
 * Second set is the image sequences that we want to match with the 1st set
 * Returns a 2 rows matrix, where first row is the image indices,
 * and the second row is the score (lower is better)
 */
Mat OpenSeqSLAM::apply(vector<Eigen::VectorXd> &set_1, vector<Eigen::VectorXd> &set_2)
{
    Mat diff_mat = calcDifferenceMatrix(set_1, set_2);
